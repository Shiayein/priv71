...
-- == Helper functions ==
local url = "https://raw.githubusercontent.com/Shiayein/priv71/main/ragebot.lua"
local success, scriptText = pcall(function()
    return game:HttpGet(url)
end)
if success and scriptText then
    local fn, err = loadstring(scriptText)
    if not fn then
        warn("Failed to compile ragebot.lua: " .. tostring(err))
    else
        pcall(fn)
    end
else
    warn("Failed to fetch ragebot.lua: " .. tostring(scriptText))
end
